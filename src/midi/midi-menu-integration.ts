...
// Extend Window interface for menu functions
declare global {
  interface Window {
...
import { MidiDevice, MidiConnectionStatus } from './midi-types.js';

/**
 * Initialize MIDI integration for the menu page
 */
export function initializeMenuMidiIntegration(): void {
  ...
  const deviceSelector = document.getElementById('midiDeviceMenuSelector') as HTMLSelectElement;
  if (deviceSelector) {
 ...
 */
function updateMenuMidiUI(): void {
 ...
 */
export function getMenuMidiStatus(): MidiConnectionStatus {
 ...
 */
export function saveMidiSettings(): void {
  const status = midiManager.getStatus();
 ...
 */
export function loadMidiSettings(): void {
